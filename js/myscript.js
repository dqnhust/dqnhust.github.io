...
function getHTMLContent(name, videoURL, platform, description, buttons) {
  ...
  var strReturn = '<div class="row" id="IDRow'+idF+'">    <div class="col-md-7" ><iframe id="IFURL'+idF" width="1px" height="1px" src="'+buttons[i].videoURL+'" frameborder="0" allowfullscreen></iframe></div>    <div class="col-md-5">        <h3>'+name+'</h3>        <h4>'+platform+'</h4>        <p>'+description+'</p>      '+buttonsHTML+'   </div></div><hr>';
  strReturn += '<script>  $("#IFURL").height( $("#IDRow"+'+idF+').height() ); </script>';
  return strReturn;
}